...
export default function SignInPage() {
  ...
  async function submit() { setBusy(true); setError(''); const supabase = createSupabaseBrowserClient(); if (!supabase) { setError('Add Supabase environment variables to enable authentication.'); setBusy(false); return } const result = signup ? await supabase.auth.signUp({ email, password }) : await supabase.auth.signInWithPassword({ email, password }); if (result.error) setError(result.error.message); else { await cartStore.mergeForUser(); router.push(returnTo) } setBusy(false) }
